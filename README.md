...
host: shared-pgbouncer:5432                │                       │ host: shared-redis:6379
                                           ▼                       ▼
                                    ┌──────────────┐        ┌──────────────┐
                                    │  PgBouncer   │        │   Redis 8    │
                                    └──────┬───────┘        │ ACL per user │
...
- `shared-db-internal` (`internal: true`, `10.250.0.0/24`) holds Postgres, PgBouncer (`10.250.0.10`), pgBackup, the one-shot `db-init` and, when enabled, Garage. pgBackup also joins the compose `default` network for rclone's internet access.
...
4. Dump roles as superuser → roles.json
...
| `garage` / `garage-init` | - | Optional local S3 (profile `garage`), internal network only |
...
| `GARAGE_BUCKET` / `GARAGE_CAPACITY_GB` / `GARAGE_DATA_DIR` | `shared-db-backups` / `100` / volume | Garage bucket, layout capacity and data location (a host path such as an external disk) |
...
- `restore.py --list [db]` shows which dates each target holds.
...
Disaster recovery restores the roles, `backup_meta` and every database of a day. Targets that are only defined in the restored `backup_meta` are searched afterwards, so databases kept on a single target are found too:
...
For a developer who wants S3 backups but keeps them local, the stack can run [Garage](https://garagehq.deuxfleurs.fr/) (`dxflrs/garage:v2.3.0`). Add to `.env`:
...
`garage-init` applies the single-node layout, creates the bucket, imports the access key and registers the backup target `garage`; it is safe to run again. Garage sits on the internal network only, so applications cannot reach the backups. Put `GARAGE_DATA_DIR` on another disk; on the same disk as Postgres it does not survive a disk failure.

An existing Garage elsewhere (for example on another server reachable over Tailscale) is just another S3 target: point `RCLONE_CONFIG_GARAGE_ENDPOINT` at it, keep `COMPOSE_PROFILES` empty and add the target with `backupctl.py target add garage --type s3 --remote garage --prefix <bucket>`.
...
└── scripts/
    ...
    └── garage_init.py          # Garage layout, bucket, key and target (garage-init)
```
...
- ⚠️ `roles.json` holds the SCRAM verifiers of every role; keep backup buckets and Drive folders private
